...
sed -i "s/^ServerName .*$/ServerName ${IPADDR}:80/g" /etc/httpd/conf/httpd.conf
sed -i "s/^ServerName .*$/ServerName ${IPADDR}:32010/g" /etc/httpd/conf.d/ssl.conf
sed -i "s/^subnet .* netmask /subnet ${IPADDR3}.0 netmask /g" /etc/cobbler/dhcp.template
